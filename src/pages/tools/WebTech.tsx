...
function TechBadge({ tech }: { tech: DetectedTech }) {
  ...
  return (
    <div
      className="flex items-start gap-3 p-4 rounded-xl transition-all hover:border-opacity-60"
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {tech.version && (
            <span className="text-xs px-1.5 py-0.5 rounded mono" style={{ background: 'rgba(255,255,255,0.07)', color: 'var(--text-muted)' }}>
              v{tech.version}
            </span>
          )}
        </div>
...
export default function WebTechPage() {
  ...
  // Group technologies by category, preserving insertion order (sorted by confidence in parser)
